...
)
{
        ...
        DXF_DEBUG_BEGIN
#endif
        char *dxf_entity_name = _strdup ("LINE");
...
)
{
        ...
        switch (inheritance)
        {
                ...
                case 2:
                        ...
                        {
                                line->dictionary_owner_soft = _strdup (p2->dictionary_owner_soft);
                        }
                        if (p2->material != NULL)
                        {
                                line->material = _strdup (p2->material);
                        }
                        if (p2->dictionary_owner_hard != NULL)
                        {
                                line->dictionary_owner_hard = _strdup (p2->dictionary_owner_hard);
                        }
                        line->lineweight = p2->lineweight;
                        if (p2->plot_style_name != NULL)
                        {
                                line->plot_style_name = _strdup (p2->plot_style_name);
                        }
                        line->color_value = p2->color_value;
                        if (p2->color_name != NULL)
                        {
                                line->color_name = _strdup (p2->color_name);
                        }
